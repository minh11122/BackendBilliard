...
const getBilliardTables = async (req, res) => {
  try {
    const club_id =
      req.user?.club_id || req.query?.club_id || req.body?.club_id;
    const { page = 1, limit = 5, search, table_type_id, status } = req.query;

    if (!club_id) {
      return res
        .status(400)
        .json({ success: false, message: "Không xác định được ID Quán." });
    }
    ...
    if (req.user?.role === "OWNER") {
      ...
      if (!isOwner)
        return res.status(403).json({
          success: false,
          message: "Bạn không có quyền thao tác trên quán này!",
        });
    }
    ...
    const getTables = async () => {
      ...
      if (status && status !== "all") {
        if (status === "Maintenance") {
          ...
          const clubTables = await BilliardTable.find({
      ...
      // API lấy danh sách bàn, rồi lấy các booking active
      ...
      for (const b of activeBookings) {
        ...
        if (!bookingMap[tid]) {
          ...
          if (
            (priority[b.status] || 0) > (priority[bookingMap[tid].status] || 0)
          ) {
            bookingMap[tid] = b;
          }
        }
      ...
      const enrichedTables = tables.map((t) => {
        //khi duyệt từng bàn, nó gắn booking đúng vào bàn đó:
...
const getBilliardTableById = async (req, res) => {
  try {
    ...
    const table = await BilliardTable.findById(id).populate(
      "table_type_id",
      "name",
    );
    if (!table)
    ...
    if (req.user?.role === "OWNER") {
      ...
      if (!isOwner)
        return res.status(403).json({
          success: false,
          message: "Bạn không có quyền xem bàn của quán khác!",
        });
    } else if (
      req.user?.role === "STAFF_CLUB" &&
      req.user.club_id !== table.club_id.toString()
    ) {
      return res.status(403).json({
        success: false,
        message: "Nhân viên không có quyền xem bàn của quán khác!",
      });
    }
  ...
  const deletePromises = urls.map((url) => {
    ...
    if (publicId) {
      return cloudinary.uploader
        .destroy(publicId)
        .catch((e) => console.error(`Lỗi xóa ảnh Cloudinary ${publicId}:`, e));
    }
  ...
  try {
    ...
    if (!club_id) {
      return res.status(403).json({
        success: false,
        message:
          "Không xác định được ID Quán (club_id). Vui lòng đăng nhập lại.",
      });
    }
    ...
    if (!table_number)
      return res
        .status(400)
        .json({ success: false, message: "Tên bàn không được để trống" });
    if (table_number.length > 50)
      return res
        .status(400)
        .json({ success: false, message: "Tên bàn quá dài (tối đa 50 ký tự)" });

    price = Number(price);
    if (isNaN(price) || price <= 0)
      return res
        .status(400)
        .json({ success: false, message: "Đơn giá phải là số lớn hơn 0" });
    ...
    if (!isValidObjectId(table_type_id)) {
      return res
        .status(400)
        .json({ success: false, message: "ID loại bàn không hợp lệ" });
    }
    ...
    if (!typeExists)
      return res
        .status(400)
        .json({ success: false, message: "Loại bàn không tồn tại" });

    const existingTable = await BilliardTable.exists({ club_id, table_number });
    if (existingTable) {
      return res.status(409).json({
        success: false,
        message: `Bàn "${table_number}" đã tồn tại trong cơ sở này!`,
      });
    }
    ...
    if (images.length > 5) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Chỉ cho phép tải lên tối đa 5 ảnh" });
    }

    const tableStatus =
      isActive === "false" || isActive === false ? "Maintenance" : "Available";
    ...
    return res
      .status(500)
      .json({ success: false, message: error.message || "Lỗi server nội bộ" });
  }
};
...
const updateBilliardTable = async (req, res) => {
  try {
    ...
    if (!isValidObjectId(id))
      return res
        .status(400)
        .json({ success: false, message: "Thiếu hoặc sai ID bàn" });
    if (!club_id)
      return res
        .status(403)
        .json({ success: false, message: "Không xác định được ID Quán." });
    ...
    if (existing.club_id.toString() !== club_id) {
      return res.status(403).json({
        success: false,
        message: "Bạn không có quyền sửa bàn của quán khác!",
      });
    }
    ...
    if (table_number !== undefined) {
      ...
      if (table_number.length > 50)
        return res.status(400).json({
          success: false,
          message: "Tên bàn quá dài (tối đa 50 ký tự)",
        });

      const tableWithSameNumber = await BilliardTable.exists({
        club_id,
        table_number,
        _id: { $ne: id },
      });
      if (tableWithSameNumber)
        return res.status(409).json({
          success: false,
          message: `Tên bàn "${table_number}" đã tồn tại trong quán!`,
        });
    ...
    if (description !== undefined) {
      ...
      if (description.length > 500)
        return res
          .status(400)
          .json({ success: false, message: "Mô tả quá dài" });
    ...
    if (tableStatus === "Maintenance" && existing.status !== "Maintenance") {
      ...
      if (activeBookings)
        return res.status(400).json({
          success: false,
          message:
            "Không thể bảo trì bàn vì đang có lịch đặt hoặc đang được chơi.",
        });
    }
    ...
    if (req.body.removedImages) {
      removedList = Array.isArray(req.body.removedImages)
    ...
    if (updateData.images.length > 5) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Bàn chỉ lưu tối đa 5 ảnh" });
    }

    const updatedTable = await BilliardTable.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    }).populate("table_type_id", "name");
    ...
    if (error.code === 11000) {
      return res.status(409).json({
        success: false,
        message: `Tên bàn "${req.body.table_number}" đã tồn tại!`,
      });
    }
    return res
      .status(400)
      .json({ success: false, message: error.message || "Lỗi cập nhật bàn" });
  }
};
...
const deleteBilliardTable = async (req, res) => {
  try {
    ...
    if (!isValidObjectId(id))
      return res
        .status(400)
        .json({ success: false, message: "Thiếu hoặc sai định dạng ID bàn" });
    if (!club_id)
      return res
        .status(403)
        .json({ success: false, message: "Không xác định được ID Quán." });
    ...
    if (table.club_id.toString() !== club_id) {
      return res.status(403).json({
        success: false,
        message: "Bạn không có quyền xóa bàn của quán khác!",
      });
    }
    ...
    if (activeMatches) {
      return res.status(400).json({
        success: false,
        message:
          "Không thể xóa bàn vì bàn đang được gán cho một trận đấu giải.",
      });
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Xóa bàn thành công" });
  } catch (error) {
    console.error("Error in deleteBilliardTable:", error);
    return res
      .status(500)
      .json({ success: false, message: "Lỗi xóa bàn: " + error.message });
  }
};
